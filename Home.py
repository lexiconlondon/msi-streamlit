...
for country in shpreader.Reader(countries_shp).records():
    ...
    if name == "Antarctica" or geom is None or geom.is_empty:
        continue
    try:
        color = "#aebaf0" if name in destination_names else "#f4f5fd"
        ax.add_geometries([geom], ccrs.PlateCarree(), facecolor=color, edgecolor="#ffffff", linewidth=0.2)
    except:
        continue
...
for _, row in df.iterrows():
 # Set minimum line thickness
    linewidth = max(row["Credits"] / 300_000, 0.5)
    ...
    ax.scatter(row["Lon"], row["Lat"],
           color='#0f477b',
           alpha=0.6,
           s=max(row["Credits"] / 10000, 20),  # 's' is marker area in points^2
           transform=ccrs.PlateCarree(),
           zorder=5)
